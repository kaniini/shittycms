<?php
#
# ShittyCMSParser, based on Markdown: A text-to-HTML conversion tool for web writers
#
# PHP Markdown & Extra
...
#
# BSD license. See ShittyCMS website for details.
#
...
define( 'MARKDOWN_PARSER_CLASS',  'ShittyCMS_Parser' );
...
class Markdown_Parser {
	...
	#
		...
		$text = $this->hashPragmaBlocks($text);
	...
	function hashPragmaBlocks($text) {
		$text = preg_replace_callback("|%pragma (.*)\(\"(.*)\"\);|", array(&$this, '_hashPragmaBlocks_callback'), $text);

		return $text;
	}
	function _hashPragmaBlocks_callback($matches) {
...
class ShittyCMS_Parser extends Markdown_Parser {
